...
import com.lichuanqi.apipassenger.redis.RedisUtil;
...
/**
 * @Author lichuanqi,  test类，这里的包名请与main/java下的包名保持一致，或者在@SpringBootTest中单独配置下。
...
public class RedisUtilTest {
    ...
    public void testExpire() throws Exception {
        ...
        // Assert.assertEquals,断言，如果两者一直，继续运行，不一致的话，抛出错误，终止运行。
        // Assert.assertNotEquals 则相反
...
//    @Ignore
    public void testIncr2() throws Exception {
        ...
        redisUtil.del(key);
        Assert.assertFalse(redisUtil.hasKey(key));
        ...
        // 一次性放入多个线程实例 ，实例 都是1秒请求一次 ，而3s内的只能允许一条。 也就是说我测试50个线程，只能3s一条
...
//                    String key = "testIncr2_17353620612";
                    ...
                    if (count == 1L) {
                        ...
                        redisUtil.expire(key, 3); // 缓存失效3s
                        System.err.println("短信发送成功===" + new Date() +",发送次数：" +sendCount.incrementAndGet());

                    } else {
                        System.err.println("访问次数快===" + new Date() + ",总调用次数" +totalCount.incrementAndGet());
                    }
                }
            });
...
//         判断等待队列里是否还有线程,等待队列为空时，关闭线程池，不再接受新的线程
